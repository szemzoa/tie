...
extern void store_KV_cache_f32_bf16_scalar(struct TIEContext *ctx, int layer_idx, int start_pos, int batch_len, int sink_len);
extern void store_KV_cache_f32_f32_scalar(struct TIEContext *ctx, int layer_idx, int start_pos, int batch_len, int sink_len);
...
extern void conv_2d_f32_f32_f32_f32_scalar(MemType *dest, const MemType *src_image, const Tensor *kernel_tensor,
					   const Tensor *bias_tensor, int H_in, int W_in, int stride, int padding);
extern void layer_norm_f32_f32_f32_f32_scalar(MemType *dest, const MemType *src, const Tensor *weight,
					      const Tensor *bias, int size, float eps);
extern void transpose_f32_scalar(MemType *dest, const MemType *src, int rows, int cols);
extern void transpose_bf16_scalar(MemType *dest, const MemType *src, int rows, int cols);
