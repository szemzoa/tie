...
void apply_residual_f32_scalar(void *acc_void, const void *residual_void, int size)
{
...
void dequantize_row_q4_k_scalar(const void *__restrict__ q_void, float *__restrict__ y, int k)
{
...
void dequantize_row_q6_k_scalar(const void *__restrict__ q_void, float *__restrict__ y, int k)
{
...
void rms_norm_f32_f32_scalar(void *__restrict O, const void *__restrict X, const Tensor *__restrict W, int size,
			     float eps)
{
	float *o = (float *)O;
	float *x = (float *)X;
...
float dot_product_q6_k_scalar(const float *x, const block_q6_k *block)
{
	float sum = 0.0f;
...
float dot_product_q4_k_scalar(const float *x, const block_q4_k *blk)
{
	const float d = fp16_to_fp32(blk->d);
	const float dmin = fp16_to_fp32(blk->dmin);
...
void mat_vec_row_q4_k_f32_scalar(const void *X, const void *w_void, void *O, int in_dim, int start_row, int end_row)
{
	...
	for (int i = start_row; i < end_row; i++) {
		...
		for (int j = 0; j < nb; j++) {
			...
			sum += dot_product_q4_k_scalar(x + j * QK_K, &w_row[j]);
		}
		o[i] = sum;
	}
}
 ...
 */
void mat_vec_row_q6_k_f32_scalar(const void *X, const void *w_void, void *O, int in_dim, int start_row, int end_row)
{
	...
	for (int i = start_row; i < end_row; i++) {
		...
		for (int j = 0; j < nb; j++) {
			// Explicitly call the scalar version of the dot product
			sum += dot_product_q6_k_scalar(x + j * QK_K, &w_row[j]);
		}
		o[i] = sum;
	}
}

void mat_vec_row_bf16_f32_scalar(const void *X, const void *w_void, void *O, int in_dim, int start_row, int end_row)
{
	float *x = (float *)X;
	float *o = (float *)O;
...
void mat_vec_row_f32_f32_scalar(const void *X, const void *w_void, void *O, int in_dim, int start_row, int end_row)
{
	float *x = (float *)X;
	float *o = (float *)O;
...
void get_embedding_row_q4_k_f32_scalar(const Tensor *W, int row_index, void *dest, int embed_dim)
{
	...
	for (int block_idx = 0; block_idx < blocks_per_row; block_idx++)
		dequantize_row_q4_k_scalar(&src[row_block_offset + block_idx], (float *)dest + block_idx * QK_K, QK_K);
}

void get_embedding_row_q6_k_f32_scalar(const Tensor *W, int row_index, void *dest, int embed_dim)
{
	int blocks_per_row = embed_dim / QK_K;
	...
	for (int block_idx = 0; block_idx < blocks_per_row; block_idx++) {
		dequantize_row_q6_k_scalar(&src[row_block_offset + block_idx], (float *)dest + block_idx * QK_K, QK_K);
	}
}
...
void get_embedding_row_bf16_bf16(const Tensor *W, int row_index, MemType *O, int embed_dim)
{
	uint16_t *src = (uint16_t *)W->mem.data;
	long long row_offset = (long long)row_index * embed_dim;
	uint16_t *dest = (uint16_t *)O->data;

	for (int i = 0; i < embed_dim; i++) {
		dest[i] = src[row_offset + i];
	}
}
